...
#include <mpif.h>
  ...
  public :: component_init               ! mct and esmf versions
  public :: component_init_post
...
contains
  ...
  subroutine component_init(Eclock, comp, comp_init, infodata, NLFilename, &
...
#ifdef ESMF_INTERFACE
  subroutine component_init(Eclock, drvcomp, comp, gridcomp_register, &
       infodata, NLFilename, seq_flds_x2c_fluxes, seq_flds_c2x_fluxes)
    ...
    do eci = 1,size(comp)
       ...
       if (init_phase == 1) then
          ...
          comp(eci)%gridcomp = ESMF_GridCompCreate(name=trim(comp(eci)%name), &
               petList=petlist, rc=rc)
          if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

          call ESMF_GridCompSetServices(comp(eci)%gridcomp, gridcomp_register, rc=rc)
          if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

          comp(eci)%import_state = ESMF_StateCreate(name=trim(comp(eci)%ntype)//" import", &
               stateintent=ESMF_STATEINTENT_IMPORT, rc=rc)
          if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

          comp(eci)%export_state = ESMF_StateCreate(name=trim(comp(eci)%ntype)//" export", &
               stateintent=ESMF_STATEINTENT_EXPORT, rc=rc)
          if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

          call ESMF_AttributeLink(drvcomp, comp(eci)%gridcomp, rc=rc)
          if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
       end if
       ...
       if (comp(eci)%iamin_compid .and. comp(eci)%present) then
          ...
          if (init_phase == 1) then 
             call ESMF_AttributeSet(comp(eci)%export_state, name="ID", &
                  value=comp(eci)%compid, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
          end if

          ! Set init_phase attribute value
          call ESMF_AttributeSet(comp(eci)%export_state, name=trim(comp(eci)%ntype)//"_phase", &
               value=init_phase, rc=rc)
          if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

          ! Convert infodata information into appropriate export state attributes
          call esmfshr_infodata_infodata2state(infodata, comp(eci)%export_state, &
          ...
          if (init_phase == 2) then ! phase 2 (only for atm for now)
             ...
             call ESMF_StateGet(comp(eci)%import_state, itemName="x2d", array=x2d_array, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
          ...
          !-----------------------------------
          call ESMF_GridCompInitialize(comp(eci)%gridcomp, &
               importState=comp(eci)%import_state, exportState=comp(eci)%export_state, &
          ...
          if (init_phase == 2) then
             call ESMF_StateGet(comp(eci)%export_state, itemName="d2x", array=d2x_array, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
          ...
          call esmfshr_infodata_state2infodata(comp(eci)%export_state, infodata, rc=rc)
          if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
       ...
       if (init_phase == 1) then
          if (comp(1)%oneletterid == 'a') call seq_infodata_getData(infodata, &
               atm_present=comp(eci)%present)
          if (comp(1)%oneletterid == 'l') call seq_infodata_getData(infodata, &
               lnd_present=comp(eci)%present)
          if (comp(1)%oneletterid == 'i') call seq_infodata_getData(infodata, &
               ice_present=comp(eci)%present)
          if (comp(1)%oneletterid == 'o') call seq_infodata_getData(infodata, &
               ocn_present=comp(eci)%present)
          if (comp(1)%oneletterid == 'r') call seq_infodata_getData(infodata, &
               rof_present=comp(eci)%present)
          if (comp(1)%oneletterid == 'g') call seq_infodata_getData(infodata, &
               glc_present=comp(eci)%present)
          if (comp(1)%oneletterid == 'w') call seq_infodata_getData(infodata, &
               wav_present=comp(eci)%present)
       end if

       if (comp(eci)%iamin_compid .and. comp(eci)%present) then

          ! The following holds for the phase 1 of all initializations
          if (init_phase == 1) then
             
             ! --- get domain from export state and initialize MCT dom_cc ---
             call ESMF_StateGet(comp(eci)%export_state, itemName="domain", array=dom_array, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
             
             call esmf2mct_init(dom_array, comp(eci)%dom_cc, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
             
             call esmf2mct_copy(dom_array, comp(eci)%dom_cc%data, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
             
             ! --- get d2x from import state and initialize x2c_cc ---
             call ESMF_StateGet(comp(eci)%import_state, itemName="x2d", array=x2d_array, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
             
             call esmf2mct_init(x2d_array, comp(eci)%x2c_cc, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
             
             call mct_aVect_zero(comp(eci)%x2c_cc)
             
             ! --- get d2x from export state and initialize c2x_cc ---
             call ESMF_StateGet(comp(eci)%export_state, itemName="d2x", array=d2x_array, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
             
             call esmf2mct_init(d2x_array, comp(eci)%c2x_cc, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
             
             call esmf2mct_copy(d2x_array, comp(eci)%c2x_cc, rc=rc)
             if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
             
             ! --- initialize MCT gsmap_cc ---
             call ESMF_AttributeGet(comp(eci)%export_state, name="gsize", value=gsize, rc=rc)
             if (rc == ESMF_SUCCESS) then
                call esmf2mct_init(d2x_array, comp(eci)%compid, comp(eci)%gsmap_cc, &
                     comp(eci)%mpicom_compid, gsize=gsize, rc=rc)
                if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
             else
                call esmf2mct_init(d2x_array, comp(eci)%compid, comp(eci)%gsmap_cc, &
                     comp(eci)%mpicom_compid, rc=rc)
                if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
             end if
          end if
  ...
  subroutine component_init_update_petlist(comp, vm)
   ...
   ! Update petlist attribute
    do eci = 1, size(comp)
       call seq_comm_petlist(comp(eci)%compid, petlist)
       call ESMF_AttributeUpdate(comp(eci)%gridcomp, vm, rootList=petlist, rc=rc)
       if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
    enddo
  ...
  subroutine component_init_post(comp, infodata) 

    !---------------------------------------------------------------
    ! Uses
    use mct_mod       , only: mct_ggrid_clean
    use seq_map_mod   , only: seq_map_init_rearrsplit
    use seq_mctext_mod, only: seq_mctext_gsmapinit, seq_mctext_avInit, &
                              seq_mctext_avExtend, seq_mctext_gGridInit
    ...
    do eci = 1,size(comp)
       if (comp(eci)%present) then
          ...
          if (comp(eci)%iamin_cplcompid) then
             ...
             ! Create gsmap_cx (note that comp(eci)%gsmap_cx all point 
             ! to comp(1)%gsmap_cx
             if (eci == 1) then
                if (iamroot_CPLID) then
                ...
                call seq_mctext_gsmapInit(comp(1)%gsmap_cc, comp(1)%compid, &
                     comp(1)%gsmap_cx, cplid, comp(1)%cplcompid)
             endif
             ...
             allocate(comp(eci)%mapper_Cc2x) 
             call seq_map_init_rearrsplit(comp(eci)%mapper_Cc2x, &
                  comp(eci)%gsmap_cc, comp(eci)%compid, &
                  comp(eci)%gsmap_cx, CPLID, &
                  comp(eci)%cplcompid)
             ...
             allocate(comp(eci)%mapper_Cx2c) 
             call seq_map_init_rearrsplit(comp(eci)%mapper_Cx2c, &
                  comp(eci)%gsmap_cx, cplid, &
                  comp(eci)%gsmap_cc, comp(eci)%compid, &
                  comp(eci)%cplcompid)
             ...
             call seq_mctext_avinit(comp(eci)%x2c_cc, comp(eci)%compid, comp(eci)%x2c_cx, &
                  cplid, comp(eci)%gsmap_cx, comp(eci)%cplcompid)
             call seq_mctext_avinit(comp(eci)%c2x_cc, comp(eci)%compid, comp(eci)%c2x_cx, &
                  cplid, comp(eci)%gsmap_cx, comp(eci)%cplcompid)

             ! Create dom_cx (note that  comp(eci)%dom_cx all point to 
             ! comp(1)%dom_cx
             if (eci == 1) then  ! create dom_cx
                ...
                call seq_mctext_gGridInit(comp(1)%dom_cc, comp(1)%compid, comp(1)%dom_cx, &
                     cplid, comp(1)%gsmap_cx, comp(1)%cplcompid)
                call seq_map_map(comp(1)%mapper_Cc2x, comp(1)%dom_cc%data, comp(1)%dom_cx%data, &
                     msgtag=comp(1)%cplcompid*100+1*10+1)
             end if

             ! Veryify other ensembles have same domain by comparing to dom_cx
             if (eci > 1) then  
                if (iamroot_CPLID) then
                ...
                call seq_mctext_gGridInit(comp(eci)%dom_cc, comp(eci)%compid, dom_tmp, &
                     cplid, comp(eci)%gsmap_cx, comp(eci)%cplcompid)
                call seq_map_map(comp(eci)%mapper_Cc2x, comp(eci)%dom_cc%data, dom_tmp%data, &
                     msgtag=comp(eci)%cplcompid*100+eci*10+1)
                if (iamin_CPLID) then
             ...
             call mct_avect_zero(comp(eci)%x2c_cc)
             call seq_map_map(comp(eci)%mapper_Cc2x, comp(eci)%x2c_cc, comp(eci)%x2c_cx, &
                  msgtag=comp(eci)%cplcompid*100+eci*10+3)

          end if ! if comp(eci)%iamin_cplcompid
       end if  ! if comp(eci)%present
    end do  ! end of eci loop

  end subroutine component_init_post

  !===============================================================================

  subroutine component_init_aream(infodata, &
       atm, lnd, ice, ocn, rof, glc, & 
       rof_c2_ocn, samegrid_ao, samegrid_al, samegrid_ro)

    !---------------------------------------------------------------
    ! Description
    ! Update (read) aream in domains where appropriate
    !
    ! Uses
    ...
    if (atm_present .and. ocn_present) then
       if (samegrid_ao) then
          dom_s  => component_get_dom_cx(atm)   !dom_ax
          dom_d  => component_get_dom_cx(ocn)   !dom_ox
          ...
          gsmap_s => component_get_gsmap_cx(ocn) ! gsmap_ox
          gsmap_d => component_get_gsmap_cx(atm) ! gsmap_ax
          dom_s   => component_get_dom_cx(ocn)   ! dom_ox
          dom_d   => component_get_dom_cx(atm)   ! dom_ax
    ...
    if (ice_present .and. ocn_present) then
       dom_s  => component_get_dom_cx(ocn)   !dom_ox
       dom_d  => component_get_dom_cx(ice)   !dom_ix
    ...
    if (rof_c2_ocn) then
       if (.not.samegrid_ro) then
          gsmap_s => component_get_gsmap_cx(rof) ! gsmap_rx
          dom_s   => component_get_dom_cx(rof)   ! dom_rx
    ...
    if (lnd_present .and. atm_present) then
       if (samegrid_al) then
          dom_s  => component_get_dom_cx(atm)   !dom_ax
          dom_d  => component_get_dom_cx(lnd)   !dom_lx

          call seq_map_map(mapper_Sa2l, av_s=dom_s%data, av_d=dom_d%data, fldlist='aream')
       else
          gsmap_d => component_get_gsmap_cx(lnd) ! gsmap_lx
          dom_d   => component_get_dom_cx(lnd)   ! dom_lx
    ...
    if (lnd_present .and. glc_present) then
       dom_s  => component_get_dom_cx(lnd)   !dom_lx
       dom_d  => component_get_dom_cx(glc)   !dom_gx
             ...
             if (present(run_barriers)) then
                if (run_barriers) then
                   ...
                   if (present(ymd) .and. present(tod)) then
                      write(logunit,107) ' rstamp ',comp(eci)%name,                &
...
107 format( A, 2i8, A, f12.4, A, f12.4 )
  ...
  subroutine component_run(Eclock, comp, infodata, &
    ...
    do eci = 1,num_inst
       if (comp(eci)%iamin_compid) then
          ...
          !----------------------------------------------
          call ESMF_GridCompRun(comp(eci)%gridcomp, &
               importState=comp(eci)%import_state, exportState=comp(eci)%export_state, &
               clock=EClock, userRc=urc, rc=rc)
          if (urc /= ESMF_SUCCESS) call ESMF_Finalize(rc=urc, endflag=ESMF_END_ABORT)
          if (rc  /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc , endflag=ESMF_END_ABORT)
          ...
          call ESMF_AttributeSet(comp(eci)%export_state, name="ID", value=comp(eci)%compid, rc=rc)
          if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
  ...
  subroutine component_final(Eclock, comp)
    ...
    do eci = 1,num_inst
       ...
       call ESMF_GridCompFinalize(comp(eci)%gridcomp, &
            importState=comp(eci)%import_state, exportState=comp(eci)%export_state, &
            userRc=urc, rc=rc)
       if (urc /= ESMF_SUCCESS) call ESMF_Finalize(rc=urc, endflag=ESMF_END_ABORT)
       if (rc  /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc , endflag=ESMF_END_ABORT)
       ...
       call ESMF_StateDestroy(comp(eci)%import_state, rc=rc)
       if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

       call ESMF_StateDestroy(comp(eci)%export_state, rc=rc)
       if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)

       call ESMF_GridCompDestroy(comp(eci)%gridcomp, rc=rc)
       if (rc /= ESMF_SUCCESS) call ESMF_Finalize(rc=rc, endflag=ESMF_END_ABORT)
