...
sub process_namelist_inline_logic {
  ...
  #######################################
  # namelist group: clm_pflotran_params #
  #######################################
  setup_logic_pflotran_interface($opts->{'test'}, $nl_flags, $definition, $defaults, $nl);
}
...
sub setup_logic_pflotran_interface {
  my ($test_files, $nl_flags, $definition, $defaults, $nl) = @_;

  add_default($test_files, $nl_flags->{'inputdata_rootdir'}, $definition, $defaults, $nl,
              'use_pflotran', 'use_pflotran'=>$nl->get_value('use_pflotran'));
  add_default($test_files, $nl_flags->{'inputdata_rootdir'}, $definition, $defaults, $nl,
              'pflotran_prefix', $nl->get_value('pflotran_prefix'));
}
...
sub write_output_files {
  ...
  if ($nl_flags->{'phys'} eq "clm4_0") {
    ...
    @groups = qw(clm_inparm ndepdyn_nml popd_streams light_streams clm_hydrology1_inparm clm_soilhydrology_inparm finidat_consistency_checks dynpft_consistency_checks clm_pflotran_inparm);
    if ( $nl_flags->{'use_lch4'}  eq ".true." ) {
