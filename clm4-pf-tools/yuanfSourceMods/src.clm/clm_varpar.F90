...
#ifdef PFTARCTIC
  integer, parameter :: exppft_arctic = 6     ! expanded # of arctic pfts (the actual should be plus the original) (FMY, 2013 July)
  integer, parameter :: mxpft       =  24+exppft_arctic     ! maximum number of PFT's for any mode; might we set some of these automatically from reading pft-physiology?
  integer, parameter :: numveg      =  16+exppft_arctic     ! number of veg types (without specific crop)
#else
...
#endif
#ifdef PFTARCTIC
  integer, parameter :: maxpatch_pft= MAXPATCH_PFT+exppft_arctic ! max number of plant functional types in naturally vegetated landunit
#else
