...
! !USES:
   ...
   use clm_time_manager, only : get_step_size
   use pftvarcon,        only : nshrub, nc3_arctic_grass,  nc3_nonarctic_grass, nc4_grass, crop, noveg
      ...
      do j=1,nlevsoi
         do fp = 1, num_methp
            ...
            ! any major changes from the Riley et al. 2011 version. (There are a few other minor ones.)
               ...
               !if (ivt(p) == nc3_arctic_grass .or. &
               if ((ivt(p) > nshrub .and. ivt(p) <= nc3_arctic_grass) .or. &    ! (FMYUAN, 2013 July)
                   ivt(p) == nc3_nonarctic_grass .or. ivt(p) == nc4_grass .or. crop(ivt(p)) == 1) then
